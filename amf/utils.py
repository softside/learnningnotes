...
import amf, amf.messaging
import datetime
import time
from types import *

class_mappings = {
        'flex.messaging.messages.RemotingMessage' : 'amf.messaging.RemotingMessage',
        'flex.messaging.messages.CommandMessage' : 'amf.messaging.CommandMessage',
        'flex.messaging.messages.AcknowledgeMessage' : 'amf.messaging.AcknowledgeMessage',
        'flex.messaging.messages.AsyncMessage' : 'amf.messaging.AsyncMessage',
        'flex.messaging.messages.ErrorMessage' : 'amf.messaging.ErrorMessage',
        }
timeoffset = None

def register_class(flash_class_fqdn, python_class):
    class_mappings[flash_class_fqdn] = python_class
...
def get_class_def(obj):
    try:
        ...
        if isinstance(obj, models.Model):
            props = [ f.name for f in obj._meta.fields ]
        else:
            props = obj.__dict__.keys()
    except ImportError:
        props = obj.__dict__.keys()

    class_name = get_as_type(obj)
    no_return_attrs = get_no_return_attrs(obj)
    member_names = [ name for name in props if not name in no_return_attrs ]
    class_def = {
            'type'           : class_name,
