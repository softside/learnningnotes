...
class AMFMessageBody(object):
    ...
    def __setup_target(self):
        if self.target == 'null':
            self.__service_name = 'null'
            self.__service_method_name = 'null'
        else:
...
class AMFProcessingError(Exception):

    def __init__(self, response_num, type, code, description, details):
    ...
    def __get_res_target(self):
        return self.response_num + RESPONSE_STATUS
    res_target = property(__get_res_target)

    def getAMFMessageBody(self):
        result = {
        ...
        return AMFMessageBody(self.res_target, 'null', result)
