...
read_func_map = {
    ...
    NULL_MARKER       : None,
    FALSE_MARKER      : 'read_boolean_false',
    TRUE_MARKER       : 'read_boolean_true',
    INTEGER_MARKER    : 'read_integer',
    DOUBLE_MARKER     : 'read_number',
    STRING_MARKER     : 'read_string',
    DATE_MARKER       : 'read_date',
    ARRAY_MARKER      : 'read_array',
    OBJECT_MARKER     : 'read_object',
    XML_DOC_MARKER    : 'read_xml',
    XML_MARKER        : 'read_xml',
    BYTE_ARRAY_MARKER : 'read_byte_array',
}
...
def read_data(input, context=None):
    ...
    if type == amf0.AVMPLUS_OBJECT_MARKER:
        type = amf0.read_byte(input)
...
def read_string(input, context):
    ...
    if inline:
        ...
        if strlen == 0:
            return ''
        str = amf0.read_string(input, context, strlen)
...
def read_byte_array(input, context):
    ...
    ref >>= 1
    if inline:
        data = amf0.read_string(input, context, ref)
...
def read_xml(input, context):
    ...
    ref >>= 1
    if inline:
        xmlStr = amf0.read_string(input, context, ref)
        doc = ET.fromstring(xmlStr)
        context.add_object_reference(doc)
    else:
        doc = context.get_object_reference(ref)
    amf.utils.logger().debug("amf3.read_xml() -- result='%s'", ET.tostring(doc, 'utf-8'))
    return doc
...
def write_data(d, output=None, context=None):
    ...
    if not context:
        context=amf.AMFMessageBodyContext()
    amf0.write_byte(amf0.AVMPLUS_OBJECT_MARKER, output) # Type Code
...
def write_null(n, output, context=None):
    ...
    amf0.write_byte(NULL_MARKER, output) # Type Code

def write_boolean(b, output, context=None):
    if b:
        amf0.write_byte(TRUE_MARKER, output) # Type Code
    else:
        amf0.write_byte(FALSE_MARKER, output) # Type Code
...
def write_string(s, output, context):
    amf0.write_byte(STRING_MARKER, output) # Type Code
...
def write_number(d, output, context):
    ...
    if isinstance(d, (int, long)) and d >= -268435456 and d <= 268435455: # check valid range for 29bits
        amf0.write_byte(INTEGER_MARKER, output) # Type Code
        _write_integer(d, output, context)
    else:
        amf0.write_byte(DOUBLE_MARKER, output) # Type Code
        amf0.write_double(d, output)

def write_date(d, output, context):
    amf.utils.logger().debug("amf3.write_date(%s)", repr(d))
    amf0.write_byte(DATE_MARKER, output) # Type Code
...
def write_dynamic_object(d, output, context):
    ...
    amf0.write_byte(OBJECT_MARKER, output) # Type Code
    key = context.get_object_reference_index(d)
    if key == -1:
        context.add_object_reference(d)
        context.add_class_def_reference(DYNAMIC_OBJECT_CLASSREF)
